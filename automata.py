...
#        return (max_del_h, max_vec)
    ...
    def run_automaton(self,N,save_data=False,report_rate=10,plot=False,random_pour=True,continous=False):
        ...
        # save data to a subfolder and mark with time 
        if save_data:
            np.savetxt('./data/E_data.csv',data_E)
            np.savetxt('./data/h_data.csv',data_h)
        ...
        #plot the graph of iterations to evaluate the complexity
        if plot and report_rate:
            iter_record=np.array(iter_record)
            np.savetxt('./data/iter_record.txt',iter_record)
